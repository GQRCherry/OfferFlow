...
function PositionModal({ open, onClose, position }: { open: boolean; onClose: () => void; position?: Position }) {
  ...
  const [parseMeta, setParseMeta] = useState({ parsedAt: position?.jdParsedAt, provider: position?.jdParserProvider, model: position?.jdParserModel })
  ...
  const runAI = async () => {
    ...
    if (!confirm(`将当前 JD 原文发送至“${provider.name}”（${provider.baseUrl}）进行提纯，确认继续？`)) return
    setParsing(true)
    try {
      const parsed = await parseJD(provider, form.jdRaw)
      setParseMeta({ parsedAt: new Date().toISOString(), provider: provider.name, model: provider.model })
  ...
  const submit = async (event: FormEvent) => {
    ...
    const payload = { cycleId: currentCycleId, companyId, title: normalizeName(form.title), department: form.department || undefined, locations: form.locations.split(/[、,，]/).map((item) => item.trim()).filter(Boolean), category: form.category || undefined, jobUrl: form.jobUrl || undefined, officialUrl: form.officialUrl || undefined, consultUrl: form.consultUrl || undefined, jdRaw: form.jdRaw || undefined, jdStructured: structured(), jdParsedAt: parseMeta.parsedAt, jdParserProvider: parseMeta.provider, jdParserModel: parseMeta.model, notes: form.notes || undefined }
...
function PositionDetail({ position, onClose, onEdit }: { position?: Position; onClose: () => void; onEdit: () => void }) {
  const { currentCycle, companies, applications } = useApp()
  const navigate = useNavigate()
  const [applicationOpen, setApplicationOpen] = useState(false)
  if (!position) return null
  const company = companies.find((item) => item.id === position.companyId)
  const readonly = currentCycle?.id === position.cycleId && currentCycle.status === 'archived'
