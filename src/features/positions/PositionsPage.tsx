...
import { createApplication, createCompany, createPosition, deletePositionSafe } from '../../db/repositories'
import { db } from '../../db/schema'
import { emptyStructuredJD } from '../../lib/constants'
import { formatDate, normalizeName, nowIso, safeExternalUrl } from '../../lib/utils'
...
function PositionModal({ open, onClose, position }: { open: boolean; onClose: () => void; position?: Position }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (position) await db.positions.put({ ...position, ...payload, updatedAt: nowIso() })
...
function PositionDetail({ position, onClose, onEdit }: { position?: Position; onClose: () => void; onEdit: () => void }) {
  ...
  const startApplication = async () => {
    ...
    const created = await createApplication({ cycleId: position.cycleId, positionId: position.id })
    toast('已创建投递与默认流程')
  ...
  return <Modal wide open onClose={onClose} title={position.title} subtitle={`${company?.name ?? '未知公司'} · ${position.locations.join(' / ') || '地点待定'}`}>
    ...
    <div className="jd-grid"><section><p className="eyebrow">RESPONSIBILITIES</p><h3>岗位职责</h3>{jd.responsibilities.length ? <ul>{jd.responsibilities.map((item) => <li key={item}>{item}</li>)}</ul> : <p className="muted">尚未整理</p>}</section><section><p className="eyebrow">REQUIREMENTS</p><h3>任职要求</h3>{jd.requirements.length ? <ul>{jd.requirements.map((item) => <li key={item}>{item}</li>)}</ul> : <p className="muted">尚未整理</p>}</section></div>
    {!!jd.preferred.length && <section className="detail-section"><p className="eyebrow">PREFERRED</p><h3>加分项</h3><ul>{jd.preferred.map((item) => <li key={item}>{item}</li>)}</ul></section>}
...
export function PositionsPage() {
  ...
  const rows = useMemo(() => cyclePositions.filter((position) => {
    ...
    return (!query || `${company?.name} ${position.title} ${position.locations.join(' ')}`.toLowerCase().includes(query.toLowerCase())) && (!companyFilter || position.companyId === companyFilter) && (appliedFilter === 'all' || (appliedFilter === 'yes') === applied)
  }).sort((a, b) => sort === 'name' ? a.title.localeCompare(b.title, 'zh-CN') : b.updatedAt.localeCompare(a.updatedAt)), [cyclePositions, companies, applications, query, companyFilter, appliedFilter, sort])
  const readonly = currentCycle?.status === 'archived'
  return <div className="page-content">
    <header className="page-heading"><div><p className="eyebrow">POSITIONS</p><h1>岗位库</h1><p>保存关注岗位，维护结构化 JD，再从这里开始投递。</p></div><div className="heading-actions"><Button variant="secondary" onClick={() => setCompanyManager(true)}><Building2 size={17} />公司与账号</Button><Button disabled={readonly} onClick={() => { setEdit(undefined); setModal(true) }}><Plus size={17} />新增岗位</Button></div></header>
    {readonly && <div className="notice">当前招聘季已归档，内容以只读方式展示。恢复招聘季后可继续编辑。</div>}
    <div className="filter-bar"><label className="search-box"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="搜索公司、岗位或地点" /></label><Select value={companyFilter} onChange={(e) => setCompanyFilter(e.target.value)}><option value="">全部公司</option>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</Select><Select value={appliedFilter} onChange={(e) => setAppliedFilter(e.target.value)}><option value="all">全部状态</option><option value="yes">已投递</option><option value="no">未投递</option></Select><Select value={sort} onChange={(e) => setSort(e.target.value)}><option value="updated">最近更新</option><option value="name">岗位名称</option></Select></div>
    {!rows.length ? <EmptyState icon={<FilePlus2 />} title="还没有岗位" description="添加第一个岗位，原始 JD 和结构化结果会一起保存在本地。" action={!readonly && <Button onClick={() => setModal(true)}>新增第一个岗位</Button>} /> : <div className="table-wrap"><table><thead><tr><th>公司 / 岗位</th><th>地点</th><th>投递状态</th><th>更新时间</th><th aria-label="操作" /></tr></thead><tbody>{rows.map((position) => { const company = companies.find((item) => item.id === position.companyId); const application = applications.find((item) => item.positionId === position.id); return <tr key={position.id} onClick={() => setParams({ position: position.id })}><td><strong>{position.title}</strong><span>{company?.name}</span></td><td><span className="inline-icon"><MapPin size={14} />{position.locations.join(' / ') || '待定'}</span></td><td><Badge tone={application ? 'success' : 'neutral'}>{application ? '已投递' : '未投递'}</Badge></td><td>{formatDate(position.updatedAt)}</td><td><div className="row-actions"><IconButton label="编辑" disabled={readonly} onClick={(e) => { e.stopPropagation(); setEdit(position); setModal(true) }}><Pencil size={15} /></IconButton><IconButton label="删除" disabled={readonly} onClick={async (e) => { e.stopPropagation(); if (confirm('确认删除该岗位？有关联投递时将拒绝删除。')) try { await deletePositionSafe(position.id); toast('岗位已删除') } catch (error) { toast(error instanceof Error ? error.message : '删除失败', 'error') } }}><Trash2 size={15} /></IconButton></div></td></tr> })}</tbody></table></div>}
