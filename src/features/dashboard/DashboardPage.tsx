...
import { differenceInCalendarDays, endOfWeek, isAfter, isBefore, parseISO, startOfDay, startOfWeek } from 'date-fns'
...
export function DashboardPage() {
  ...
  return <div className="page-content">
    <header className="page-heading"><div><p className="eyebrow">{currentCycle?.name}</p><h1>掌握这一周，推进每一步。</h1><p>聚焦当前招聘季的进展、临近日程和需要关注的流程。</p></div><Button onClick={() => setEventOpen(true)}><CalendarPlus size={17} />新增日程</Button></header>
    ...
    <div className="dashboard-grid">
      <section className="panel focus-panel"><header><div><p className="eyebrow">THIS WEEK</p><h2>本周事项</h2></div><Badge tone="accent">{weekEvents.length} 项</Badge></header>
        {!weekEvents.length ? <EmptyState icon={<CalendarClock />} title="本周暂时空闲" description="安排测评、笔试或面试后，会统一显示在这里。" action={<Button size="sm" variant="secondary" onClick={() => setEventOpen(true)}>新增日程</Button>} /> : <div className="timeline-list">{weekEvents.map((event) => <button key={event.id} onClick={() => setSelectedEvent(event)}><div className="date-block"><strong>{relativeDay(event.startAt)}</strong><span>{event.allDay ? '全天' : event.startAt?.slice(11, 16)}</span></div><div className="timeline-line"><span /></div><div className="grow"><strong>{event.title}</strong><p>{describe(event) || eventTypeLabels[event.type]}</p></div><Badge tone={event.completed ? 'success' : 'neutral'}>{event.completed ? '已完成' : eventTypeLabels[event.type]}</Badge></button>)}</div>}
      </section>
      <section className="panel attention-panel"><header><div><p className="eyebrow">NEEDS ATTENTION</p><h2>需要关注</h2></div><AlertCircle size={20} /></header>
        {!stale.length ? <div className="all-clear"><CheckCircle2 size={28} /><strong>没有停滞流程</strong><p>活跃投递均在 7 天内更新过。</p></div> : <div className="attention-list">{stale.slice(0, 6).map((application) => { const position = positions.find((item) => item.id === application.positionId); const company = companies.find((item) => item.id === position?.companyId); return <Link key={application.id} to={`/applications/${application.id}`}><span className="attention-dot" /><div><strong>{company?.name} · {position?.title}</strong><p>{currentStage(application)?.name} · 已 {differenceInCalendarDays(today, parseISO(application.updatedAt))} 天未更新</p></div><ArrowUpRight size={16} /></Link> })}</div>}
      </section>
    </div>
    <CalendarPanel events={cycleEvents} onCreate={(date) => { setInitialDate(date); setEventOpen(true) }} onSelect={setSelectedEvent} />
