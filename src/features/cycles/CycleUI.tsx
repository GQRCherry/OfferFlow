...
import { archiveCycle, createCycle, restoreCycle } from '../../db/repositories'
...
export function CycleModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { toast } = useApp()
  ...
  return <Modal open={open} onClose={onClose} title="新建招聘季" subtitle="岗位、投递和日程将按招聘季隔离。">
    <form className="form-stack" onSubmit={submit}>
      ...
      <div className="modal-actions"><Button type="button" variant="ghost" onClick={onClose}>取消</Button><Button disabled={saving}>{saving ? '创建中…' : '创建招聘季'}</Button></div>
    </form>
  </Modal>
}
...
export function Onboarding() {
  ...
  return <main className="onboarding-shell">
    <section className="onboarding-card">
      ...
      <p className="eyebrow">LOCAL-FIRST JOB HUNT OS</p>
...
export function CycleManager({ open, onClose }: { open: boolean; onClose: () => void }) {
  const { cycles, currentCycleId, setCurrentCycleId, toast } = useApp()
