...
import { clearAllSecrets, readLLMSecret, saveLLMSecret } from '../../security/secrets'
import type { LLMProviderConfig, Theme } from '../../types/domain'
...
export function SettingsPage() {
  ...
  const saveProvider = async (event: FormEvent) => {
    ...
    const next: LLMProviderConfig = { id, name: form.name.trim() || 'OpenAI-compatible', providerType: 'openai_compatible', baseUrl: form.baseUrl.trim().replace(/\/$/, ''), model: form.model.trim(), apiKeyRef: form.storage === 'local' ? id : undefined, isDefault: true, createdAt: provider?.createdAt ?? timestamp, updatedAt: timestamp }
    await db.transaction('rw', db.llmProviderConfigs, db.llmSecrets, async () => {
      ...
      await db.llmProviderConfigs.put(next)
      if (form.storage === 'local' && form.apiKey) await saveLLMSecret(id, form.apiKey)
      if (form.storage === 'session') {
        sessionStorage.setItem(`offerflow.llm.${id}`, form.apiKey)
        await db.llmSecrets.delete(id)
      }
    })
    toast('AI Provider 已保存')
  }
  const runTest = async () => {
    if (!provider) return toast('请先保存 Provider', 'info')
    setTesting(true)
  ...
  return <div className="page-content narrow-page">
    <header className="page-heading"><div><p className="eyebrow">PREFERENCES</p><h1>设置</h1><p>主题、默认招聘季、AI Provider 与敏感数据管理。</p></div></header>
